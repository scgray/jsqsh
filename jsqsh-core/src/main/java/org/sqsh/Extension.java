...
/**
 * Definition of a jsqsh extension. All extensions that are known to jsqsh, either by explicitly importing (via the
 * <code>\import</code> command) or by way of being defined in <code>$JSQSH_HOME/extensions</code> or
 * <code>$HOME/.jsqsh/extensions</code> will have a definition, however just having a definition doesn't mean that the
 * extension has been loaded. That is, the jars may not yet have been loaded, the commands may not yet be defined, etc.
 *  The extension's {@link #isLoaded()} method may be used to determine if it has been loaded and the {@link
 * #load(SqshContext, ClassLoader)} method will cause it to be loaded (note that only the {@link ExtensionManager} is
 * allowed to load an extension
 */
public class Extension {
    ...
    /**
     * The short name of the extension.  This corresponds to the name of the directory in which it is found
     */
    ...
    /**
     * The classpath used to load the extension. This will be null until the point at which the extension is loaded.
     * For extensions that are loaded in response to a JDBC driver being loaded, this will not reflect the classpath
     * that was used to load the JDBC driver as well.
     */
    private String[] classpath;

    /**
     * A list of driver definitions that can trigger the extension to be loaded. This will never be null, but may be
     * empty.
     */
    private String[] drivers = EMPTY_STRING_ARRAY;

    /**
     * If not null, the name of a class implementing {@link ExtensionConfigurator} that will be invoked when the
     * extension is loaded in order to configure the extension.
     */
    private String configClass = null;

    /**
     * The classloader used to load the extension. This will be null if the extension has not yet been loaded.
     */
    private ClassLoader classloader;

    /**
     * The properties from <code>jsqsh-extension.conf</code> that were used to configure the extension.  Note that these
     * are kept here for reference only, and the configuration properties are turned into discrete class member
     * variables (e.g. isLoadOnStart) when the extension is created.
     *
     * <p>One interesting note: the Extension class is passed to the
     * {@link ExtensionConfigurator} when it is configured and, as a result, these properties are available to it, so it
     * is possible to pass additional configuration information via the jsqsh-extensions.conf into the configurator
     * class if desired.
     */
    ...
    /**
     * True if this extension has been loaded.  Note that this will be true even if the extension threw an exception and
     * failed to load.
     */
    private boolean isLoaded = false;

    /**
     * If not null, contains the path to a script that will be executed when the extension is loaded, that is expected
     * to return a classpath that should be used to load the extension jars.
     */
    ...
    /**
     * @return The classpath that is used for the extension.  This will return null if the extension has not yet been
     *         loaded.
     */
    ...
    /**
     * @return The classloader used to load the extension.  This will return null if the extension has not yet been
     *         loaded.
     */
    ...
    /**
     * @return The set of drivers that can trigger this to be loaded. If none are registered an empty array is
     *         returned.
     */
    ...
    /**
     * Returns whether or not this extension will be triggered to be loaded by a specific JDBC driver.
     *
    ...
    /**
     * @return The name of the class that will be used to configure the extension when it is loaded or null if no class
     *         is configured.
     */
     ...
     */
    protected void load(SqshContext context, ClassLoader parentClassloader) throws ExtensionException {
        ...
        for (String scriptPath : scriptPaths) {
            ...
            if (scriptPath.endsWith("/*") || scriptPath.endsWith("\\*")) {
                ...
                File[] jars = dir.listFiles((dir1, name) -> name.endsWith(".jar") || name.endsWith(".zip"));
        ...
        if (vars.exists()) {
            ...
            try (FileInputStream in = new FileInputStream(vars)) {
                vm.load(this.classloader, vars.toString(), in);
            } catch (IOException e) {
                throw new ExtensionException(
                        "Failed to read variable definition file: " + vars.toString() + ": " + e.getMessage());
            }
        }
        ...
        if (configClass != null) {
            ...
            try {
                ...
                throw new ExtensionException(
                        "Failed to create configuration class " + configClass + ": " + e.getMessage(), e);
            }
            try {
                configurator.configure(context, this);
            } catch (Exception e) {
                throw new ExtensionException(
                        "Extension configurator " + configClass + " failed due to: " + e.getMessage(), e);
            }
        }
    }
