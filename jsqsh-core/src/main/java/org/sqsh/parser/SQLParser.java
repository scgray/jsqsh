...
/**
 * The SQLParser is a very very rudamentary parser for traversing a SQL statement or batch of SQL statements looking for
 * "interesting" items that may be found in the SQL. The parser does not fully understand any SQL dialect, but
 * recognizes many keywords and aspects of queries.
 */
public class SQLParser {
    
    /**
     * This is a list of keywords that we treat as the beginning of a statement.
     */
    private static final SQLStatementSet STATEMENTS = new SQLStatementSet(
            "ALTER",
            "BEGIN",
            "BREAK",
            "COMMIT",
     ...
     *
     * @param listener The object to be notified when the parser hits "interesting" points in the SQL.
     */
    ...
    /**
     * Method made available to SQLParserListener's to request a handle to the tokenizer used for parsing. It is
     * important to note that any tokens consumed by this tokenizer will no longer be available to the SQLParser unless
     * you unget them.
     *
        ...
        while (keyword != null) {
            ...
            if (statement != null) {
                ...
                if (CLAUSES.contains(keyword) && inDML) {
                    ...
                    // To prevent false positives on clauses, we only recognize them if they appear as part of a
                    // DML statement. For example a FROM may also be part of some other part of the SQL dialect, so
                    // we will only pay attention if it comes after a SELECT/INSERT/UPDATE/DELETE.
    ...
    /**
     * Called after the tokenizer hits a DELETE statement and attempts to determine the table name that is being
     * deleted.
     */
    ...
    private void doSubquery() {
        ...
        // Keep track of how deeply nested we were when we entered the subquery.
        ...
        String token = tokenizer.next();
        while (token != null) {
            // If we hit a close paren and it knocks us back to the depth at which the subquery started, then we are finished.
    ...
    private void doFrom() {
        ...
        while (token != null && !done) {
            ...
            // The starting token is expected to be the name of an object that we are selecting from.
            ...
            if (Character.isLetter(ch) || ch == '_') {
                ...
                if (token != null && "WHERE".compareToIgnoreCase(token) != 0
                        && (token.charAt(0) == '_' || Character.isLetter(token.charAt(0)))) {
                ...
                // of a sub-select.
                while (token != null && "WHERE".compareToIgnoreCase(token) != 0
                        && !token.equals(",") && tokenizer.getParenCount() >= parenDepth) {
                    token = tokenizer.next();
                }
    ...
    /**
     * Called after the tokenizer hits an INSERT/UPDATE statement and attempts to determine the table name that is being
    ...
    /**
     * Called after the tokenizer hits a EXEC[UTE] statement and attempts to determine the procedure name that is being
     * deleted.
     *
    ...
    private void doExecuteProcedure() {
        ...
        // If there is an '@' sign in front of the next token, it is either the @return_code or the procedure's name
        // in a @variable.
    ...
    /**
     * Called when the parser has hit an object it thinks is the name of a database object. This purpose of this method
     * is to try to see if the object is a multi.part.name and will take care of sucking in the rest of the name.
     *
    ...
    /**
     * A SQLStatement is a sequence of keywords (tokens) that, put together, represent a logical SQL statement.
     */
    private static class SQLStatement {
         ...
         *
         * @param keywords This is a white-space delimited sequence of keywords that, together, make up the SQL
         *         Statement.
         */
        ...
        /**
         * Compares a provided keyword against the statement definition. In the case where the SQLSTatement involves
         * more than one keyword, the provided {@link SimpleSQLTokenizer} will be polled to fetch the next available
         * keyword (multiple times if necessary) to perform a full comparison.  If the match is successful then the
         * tokenizer will be left pointing at the next token immediately following the SQL statement. If the match is
         * unsuccessful then the state of the tokenizer will be unaltered.
         *
         * @param word The word to compare.
         * @param tokenizer If this SQLStatement comprises more than one keyword then the tokenizer will be used
         *         to pull the next available keyword.
    ...
    private static class SQLStatementSet {
        ...
        public SQLStatementSet(String... statements) {
            Arrays.stream(statements).forEach(this::add);
        }
