...
/**
 * The ShellManager provides key JNI functionality that sqsh needs to do its job. Primarily, its job is
 * to spawn external processes. The {@link java.lang.ProcessBuilder} interface is fine and dandy and
 * all but unfortunately it is impossible to spawn a process that is connected to the actual console
 * TTY--every process that is spawned must have its input and output connected to a JVM stream.
 */
public class ShellManager {
    ...
    private ShellManager() {
        ...
        if (os.startsWith("Windows")) {
            ...
            if ("x86".equals(arch)) {
                ...
                if ((str != null && str.contains("64")) || System.getenv("ProgramFiles(x86)") != null) {
                    arch = "amd64";
                }
            }
    ...
    /**
     * Called when the manager should be reset to utilize its default shell based upon the operating system
     * that the manager is being run with.
     */
    ...
    /**
     * Returns whether the ShellManager is using its JNI interface or its Java interface. The JNI interface
     * allows command executed by the shell to have access to the actual console TTY. When run through the
     * java interface commands that expect to control the terminal (such as text editors and programs like "more")
    ...
    /**
     * Spawns a command string as a sub-process that expects to receive its input from the JVM. The input to the
     * sub-process can be retrieved via {@link Shell#getStdin()}. The stdout and stderr of the sub-process is
     * send to the calling terminal.
     *
     * @param cmd The command to be executed in the sub-shell
     * @return A handle to the spawned process.
     */
    public Shell pipeShell(String cmd) throws ShellException {
        ...
        final OutputGobbler oc = new OutputGobbler(p.getInputStream());
        oc.start();
    ...
    /**
     * Spawns a shell to execute a command string as a sub-process that expects to receive its input from the JVM.
     * The input to the sub-process can be retrieved via {@link Shell#getStdin()}. The stdout and stderr of the
     * sub-process is send to the calling terminal.
     *
     * @param cmd The command to be executed in the sub-shell
     * @return A handle to the spawned process.
     */
    public Shell detachShell(String cmd) throws ShellException {
     ...
     * @return A handle to the spawned process.
     */
    public Shell readShell(String cmd, boolean combineError) throws ShellException {
        String[] argv = getShellExecution(cmd);
        ...
        try {
         ...
         // If we aren't going to return the error stream as part of the string we are returning, then send it
         // to the session's error stream.
        if (!combineError) {
    ...
    /**
     * Spawns an external process that is not attached to the current process in any way shape or form.
     *
     * @param cmd The command to execute.
     * @return A handle to the shell.
     * @throws ShellException Thrown if there is a problem.
     */
    public static native Shell exec(String[] cmd) throws ShellException;

    /**
     * Spawns a sub-shell of a process that is has its own stdout and stderr, but which receives its stdin
     * from the JVM.
     *
     * @param cmd The command to execute.
     * @return A handle to the spawned process.
     ...
     *
     * @return The width of the console in characters or -1 if the width cannot be determined.
     */
     ...
     *
     * @return The width of the screen or -1 if no screen width is available.
     */
     ...
     */
    protected static native int waitPid(long pid) throws InterruptedException;
     ...
     */
    protected static native void writeByte(long fd, int b) throws IOException;
     ...
     */
    protected static native void writeBytes(long fd, byte[] bytes, int offset, int len) throws IOException;
     ...
     */
    protected static native void close(long fd) throws IOException;
     ...
     */
    private static class OutputGobbler extends Thread {
    ...
    static class InputGobbler extends Thread {
        OutputStream os;
