...
public class HelpTopic implements Comparable<HelpTopic> {
     ...
     * @param helpLocation The location of the full Markdown help text for the topic. The location specified
     *         must be readable using the default jsqsh classloader (i.e. it should be contained in a jar somewhere).
    ...
    public String getHelp() {
        String resourcePath = helpLocation == null
                ? DEFAULT_HELP_LOCATION + "/" + (topic.charAt(0) == '\\' ? topic.substring(1) : topic) + ".md" : helpLocation;
    ...
    /**
     * Sets where the help text for the topic can be found. The location specified must be readable using the default
     * jsqsh classloader (i.e. it should be contained in a jar somewhere).  If this value is null, then
     * <i>org/sqsh/docs/topicName.md</i> is assumed. If the topic name started with a backslash, then the backslash is
     * removed.
     *
    ...
    /**
     * Compares the names of two topics. This method is provided primarily to allow for easy sorting of topics on
     * display.
     *
    ...
    /**
     * Compares two topics for equality. topics are considered equal if their names match.
     *
    ...
    /**
     * Returns a hash value for the topics. The hash code is nothing more than the hash code for the command name
     * itself.
     *
