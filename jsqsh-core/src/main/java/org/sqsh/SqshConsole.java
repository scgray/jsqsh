...
/**
 * Wrapper around the JLine LineReader console that provides some additional functionality.
 */
public class SqshConsole {
    /**
     * Enumerates the reason that a call to readLine accepted its input and returned to the caller
     */
    public enum AcceptCause {
        /**
         * Means that the user's input "normally" was complete.  That is, they ran a command or the terminated the
         * statement with a terminator
         */
        NORMAL,
        /**
         * The user explicitly asked for the statement to be executed (typically via a "CTRL-G").
         */
        EXECUTE
    }
    ...
    public SqshConsole(SqshContext context) {
        ...
        // This installs a widget that intercepts \n and attempts to determine if the input is "finished" and should
        // be run or if the user should keep typing the current statement. I hate that I have to do this for
        // all of the keymaps that JLine3 has...
    ...
    public LineReader getSimpleLineReader() {
        if (simpleLineReader == null) {
            simpleLineReader = LineReaderBuilder.builder().appName("jsqsh").terminal(newTerminal()).build();
        }
    ...
    private Terminal newTerminal() {
        ...
        try {
            ...
            System.err.println("Unable to create terminal: " + e.getMessage() + ". Falling back to dumb terminal");
            try {
                terminal = TerminalBuilder.builder().dumb(true).build();
            } catch (IOException e2) {
                System.err.println("Unable to create dumb terminal: " + e2.getMessage() + ". Giving up");
    ...
    /**
     * When called after <code>readLine()</code> returns normally, this indicates the reason that it returned (@see
     * AcceptCause}.
     *
    ...
    /**
     * @return true if multi-line editing is enabled when JLine is in use (during interactive mode)
     */
     ...
     *
     * @param multiLineEnabled true if multi-line editing is enabled, false otherwise.
     */
    ...
    /**
     * Sets the editing mode for the console reader.  Logically supported names are 'vi' and 'emacs', however any valid
     * keymap supported by JLine3 is supported.
     *
    ...
    public String getEditingMode() {
        ...
        if (currentKeyMap == keyMaps.get(LineReader.VICMD) || currentKeyMap == keyMaps.get(LineReader.VIOPP)) {
            return "vi";
    ...
    /**
     * JLine widget that is used to intercept ENTER/RETURN (CTRL-M) and determine if it should "accept" the input,
     * meaning that the user's done typing input and jsqsh should use it, or if it should continue prompting the user
     * for input.
     */
