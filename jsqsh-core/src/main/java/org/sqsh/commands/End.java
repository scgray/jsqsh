...
/**
 * Implements the \end command to end the current session and switch to another session.
 */
    ...
    public int execute(Session session, SqshOptions opts) throws Exception {
        ...
        if (options.arguments.size() == 1) {
            try {
                ...
                session.err.println("Invalid session id '" + options.arguments.get(0) + "'");
                return 1;
            }
            target = session.getContext().getSession(targetSession);
            if (target == null) {
                session.err.println("Specified next session id '" + options.arguments.get(0) + "'" + " does not exist");
                return 1;
            }
        }
