 ...
 */
public class Create extends Command implements DatabaseCommand {
    ...
    public int execute(Session session, SqshOptions opts) throws Exception {
        ...
        // Turn the supplied object name, which could contain separate parts (e.g. db.dbo.table), into component parts.
        ObjectDescription obj = SQLTools.parseObjectName(options.arguments.get(0));
        try {
            int maxNameLength = 0;
            DatabaseMetaData meta = conn.getMetaData();
            String catalog = (obj.getCatalog() == null ? SQLTools.getCatalog(conn) : obj.getCatalog());

            // We make two passes through the metadata. The first is to find out how wide the widest column name is.
            ResultSet res = meta.getColumns((catalog == null ? "%" : catalog),
                    (obj.getSchema() == null ? "%" : obj.getSchema()), obj.getName(), "%");
            while (res.next()) {
                String name = SQLTools.quoteIdentifier(res.getString("COLUMN_NAME"));
            ...
            sb.append("CREATE TABLE ").append(options.arguments.get(0)).append(linesep);
            sb.append('(').append(linesep);
            res = meta.getColumns((catalog == null ? "%" : catalog),
                    (obj.getSchema() == null ? "%" : obj.getSchema()), obj.getName(), "%");
            ...
            while (res.next()) {
                ...
                if (typeId == Types.NUMERIC || typeId == Types.DECIMAL) {
                    ...
                    typeBuf.append(')');
                        ...
                        // || typeId == Types.NCHAR
                        || typeId == Types.BINARY || typeId == Types.VARBINARY || typeId == Types.VARCHAR
