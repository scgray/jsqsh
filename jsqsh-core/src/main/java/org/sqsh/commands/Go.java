...
public class Go extends Command {
    ...
    protected static class Options extends SqshOptions {
        @OptionProperty(option = 'm', longOption = "display-style", arg = REQUIRED, argName = "style",
                description = "Sets the display style for output")
        public String style = null;

        @OptionProperty(option = 'i', longOption = "insert", arg = REQUIRED, argName = "table",
    ...
    public int execute(Session session, SqshOptions opts) throws Exception {
        ...
        Map<String, String> origVariableValues = new HashMap<>();

        if (conn == null) {
            session.err.println("You are not currently connected to a database or another queryable data source. "
                    + "Type 'help \\connect' for " + "details");
            return 1;
        }
        ...
        // If we are being asked to generate INSERT statements then we need to switch the NULL display to be a
        // form of NULL that works in SQL.
        ...
        try {
            // If the user has asked for a crosstab, I go through some wacky chicanery here. To do the crosstab there
            // is a "special" renderer called the PivotRenderer. This one isn't registered or created like a normal
            // renderer. Instead, we create it, and temporarily register it with the renderer manager under a
            // "random" name, then we set this name as the display style to use.  At the end, the renderer is
            // unregistered.
            if (options.crosstab != null) {
                String[] parts = options.crosstab.split(",");
                if (parts.length != 3) {
                    session.err.println("--crosstab (-c) requires three values vcol,hcol,dcol " + "(vertical column, horizontal column, data column");
                    return 1;
                }
                Renderer currentRenderer = renderMan.getRenderer(session);

                // Create the renderer and give it a random name
                final PivotRenderer pivot = new PivotRenderer(session, renderMan, currentRenderer, parts[0], parts[1], parts[2]);
                final String rendererName = UUID.randomUUID().toString();

                // Register a factory with the renderer manager that will serve up our renderer whenever it is asked for
                rendererFactory = name -> {
            ...
            // If we set the connection's query timeout property, then reset it to zero when we are finished.
            if (options.queryTimeout > 0) {
        ...
        if (options.repeat > 1) {
            ...
            System.out.println(options.repeat + " iterations (total " + TimeUtils.millisToDurationString(endTime - startTime)
