 ...
 */
public class Select extends Command implements DatabaseCommand {

    private static class Options extends SqshOptions {
        @OptionProperty(option = 'p', longOption = "print", arg = NONE,
                description = "Print statement to screen, do not append to SQL buffer")
        public boolean printOnly = false;

        @OptionProperty(option = 'n', longOption = "natural-join", arg = NONE,
    ...
    public int execute(Session session, SqshOptions opts) throws Exception {
        ...
        for (int i = 0; i < descriptions.length; i++) {
            try {
                ...
                session.err.println("Failed to fetch column descriptions for " + "table " + options.arguments.get(i) + ": " + e.getMessage());
                return 1;
            }

            // If no columns where found for a given table, that means it couldn't be found.
            if (descriptions[i].columns.length == 0) {
                session.err.println("Unable to find description of table " + options.arguments.get(i));
                return 1;
            }
        }
        ...
        for (int i = 0; i < descriptions.length; i++) {
            ...
            for (int j = 0; j < table.columns.length; j++) {
                if (i > 0 || j > 0) {
                    query.append(',').append(linesep).append("       ");
                }
                query.append(table.alias).append('.').append(SQLTools.quoteIdentifier(table.columns[j].name));
            }
        }
        ...
        for (int i = 0; i < descriptions.length; i++) {
            ...
            query.append(table.name).append(' ').append(table.alias);
        }
        ...
        if (descriptions.length > 1) {
            ...
            if (join.length() > 0) {
                query.append(linesep).append(" WHERE ").append(join);
            }
        }
    ...
    /**
     * Attempts to fetch a join clause by asking the database about primary/foriegn key relationships between tables.
     *
     ...
     * @throws SQLException
     */
    private String getKeyJoin(Session session, Table[] tables, String linesep) throws SQLException {
        ...
        for (int t1 = 0; t1 < tables.length; ++t1) {
            ...
            for (int t2 = 0; t2 < tables.length; ++t2) {
                ...
                ResultSet rs = meta.getCrossReference(table1.catalog, table1.schema, table1.tableName, table2.catalog, table2.schema, table2.tableName);
                while (rs.next()) {
                    if (joinCount > 0) {
                        join.append(linesep).append("   AND ");
                    }
                    join.append(table1.alias).append('.').append(SQLTools.quoteIdentifier(rs.getString(4))).append(" = ").append(table2.alias).append('.').append(SQLTools.quoteIdentifier(rs.getString(8)));
                    ++joinCount;
                }
            }
        }
    ...
    /**
     * Generates a "natural" join between tables...that is, it attempts to join all columns of the same name and
     * datatype.
     *
            ...
            for (int t2 = t1 + 1; t2 < tables.length; ++t2) {
                ...
                for (Column c1 : table1.columns) {
                    for (Column c2 : table2.columns) {
                        if (c1.name.equals(c2.name) && c1.type == c2.type) {
                            if (joinCount > 0) {
                                join.append(linesep).append("   AND ");
                            }
                            join.append(table1.alias).append('.').append(SQLTools.quoteIdentifier(c1.name)).append(" = ").append(table2.alias).append('.').append(SQLTools.quoteIdentifier(c2.name));
                            ++joinCount;
                        }
                    }
                }
     ...
     */
    private Table describe(Session session, String name) throws SQLException {
        Connection conn = session.getConnection();
        DatabaseMetaData meta = conn.getMetaData();
        SQLTools.ObjectDescription nameDescription = SQLTools.parseObjectName(name);
        Table table = new Table();
        table.name = name;
        ResultSet result = meta.getColumns(nameDescription.getCatalog(), nameDescription.getSchema(), nameDescription.getName(), "%");
