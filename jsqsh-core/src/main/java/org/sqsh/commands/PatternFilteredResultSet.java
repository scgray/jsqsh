...
/**
 * Silly wrapper class to allow for pattern filters on columns of an existing result set.
 */
public class PatternFilteredResultSet implements ResultSet {
    ...
    protected static class Filter {
        ...
        public Filter (int col, Pattern pattern) {
    ...
    public PatternFilteredResultSet (ResultSet rs, Object ... args) {
        ...
        while (i < args.length) {
            ...
            int col = (Integer) args[i];
            String pattern = (String) args[i+1];
            
            if (pattern != null && pattern.length() > 0) {
                
                filters.add(new Filter(col, toRegex(pattern)));
            }
            i += 2;
        }
    }
    ...
    private boolean rowMatches() throws SQLException {
        
        for (int i = 0; i < filters.size(); i++) {
            
            Filter f = filters.get(i);
            String val = getString(f.col);
            ...
            if (! f.pattern.matcher(val).matches()) {
