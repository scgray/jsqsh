...
/**
 * The PivotRenderer is a "special" renderer that takes an incoming result set and pivots it based upon specific column
 * in the original result set. For example, given a result set of:
 * <pre>
 * +-------+-----------+----------+
 ...
 * +-------+-----------+----------+
 * </pre>
 * You can indicate which column becomes the row header (called the vertical header), which becomes the column header
 * (the horizontal header) and which provides the data. In this case, choosing:
 * <pre>
 *     Vertical   = DAYOFWEEK
 *     Horizontal = STATE
 *     Data       = SALES
 ...
 * +-----------+---------+----------+----------+
 * </pre>
 * In order to do this, it is important to note that the PivotRenderer must buffer all of the data for the final table
...
public class PivotRenderer extends Renderer {
    ...
    private final Map<String, Integer> valueToRowIdx = new LinkedHashMap<>();
    // Maps a value in the column (horizontal) header to which row in the
    // final table contains the values
    private final Map<String, Integer> valueToColIdx = new HashMap<>();
    ...
    private static class ColumnInfo {
        ...
        public ColumnInfo(ColumnDescription desc, int idx, String name) {
            this.desc = new ColumnDescription(name, desc.getWidth(), desc.getAlignment(), desc.getOverflowBehavior(),
                    desc.isResizeable());
            this.idx = idx;
        }
    }
     ...
     * @param out A renderer that performs the actual final display of the results that the pivot renderer
     *         produces
     * @param vertColName The name or number (indexed from 1) of the column that will produce the row headers
     * @param horizColName The name or number of the column that will produce the horizontal column headers
     * @param dataColName The column that will provide the data for the final table.
     */
    public PivotRenderer(Session session, RendererManager renderMan, Renderer out, String vertColName,
            String horizColName, String dataColName) {
    ...
    public void header(ColumnDescription[] columns) {
        ...
        // If any of the columns were not found, then don't try to display anything during this round of results.
    ...
    public boolean flush() {
        ...
        if (table != null) {
            Arrays.sort(descriptions, 1, descriptions.length, Comparator.comparing(o -> o.desc.getName()));
    ...
    private int findColumn(ColumnDescription[] columns, String name) {
        if (isNumber(name)) {
            ...
            if (val < 1 || val > columns.length) {
                session.err.println("Column #" + val + " does not exist in result set " + "(which has " + columns.length
                        + " columns)");
                return -1;
            }
            return val - 1;
        ...
        // TODO: When matching column names this should really happen according to the same rules that are used by
        //  the database. For convenience I'll just to a case insenstive match for now.
