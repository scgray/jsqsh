...
/**
 * Implements a renderer that is similar to mysql's or henplus's in that it will attempt to analyze all of the available
 * data to perfectly display all columns without wasting too much space.
 */
public class PerfectPrettyRenderer extends AbstractPrettyRenderer {
    ...
    /**
     * Minimum size we will shrink a column that resists resizing. For the most part we expect numbers to be the ones
     * that resist resizing, so we will allow them to shrink a bit smaller because 10 would still be a big-ish number.
     */
    ...
    protected PerfectPrettyRenderer(Session session, RendererManager renderMan, boolean hasOuterBorder) {
    ...
    /**
     * This method is called just prior to display and attempts to adjust the width of specific columns so that they
     * will try to stay within the width of the screen overall.
     */
    private void perfectWidth() {

        // Attempt to grab the width of the console. If we can't get it, then don't go any further.
        int screenWidth = session.getScreenWidth();
        if (screenWidth <= 0) {
            return;
        }

        // If borders are enabled, then pre-add the space for the leading "| " and the trailing " |".
        int totalWidth = (hasOuterBorder ? 4 : 0);
        for (int i = 0; i < columns.length; i++) {

            // For all columns after the first, account for " | " before the column value is printed.
            if (i > 0) {
                totalWidth += 3;
            }
        ...
        while (nShrinks > 0 && totalWidth > screenWidth) {
            nShrinks = 0;
            for (int i = 0; totalWidth > screenWidth && i < columns.length; i++) {
                ColumnDescription col = columns[i];
                // Can be resized
                if (col.isResizeable()
                        // But not shorter than the column header
                        && (col.getName() == null || col.getWidth() > col.getName().length())
        ...
        // Pass #2:
        //
        // Try again, but allow shrinking smaller than the column header length if we have to.
        nShrinks = 1;
        while (nShrinks > 0 && totalWidth > screenWidth) {
            nShrinks = 0;
            for (int i = 0; totalWidth > screenWidth && i < columns.length; i++) {
        ...
        // Pass #3:
        //
        // Ok, we have shrunk all of the resizeable columns, now we have to stop honoring the ones that
        // requested no resizing.
        nShrinks = 1;
        while (nShrinks > 0 && totalWidth > screenWidth) {
            nShrinks = 0;
            for (int i = 0; totalWidth > screenWidth && i < columns.length; i++) {
        ...
        // There are no more passes now. If we didn't shrink enough...well, we're just S.O.L.
    }
